...
export default function App() {
  ...
  const [venuePhotos, setVenuePhotos] = useState<VenuePhoto[]>(() => {
    try {
      const cached = localStorage.getItem('chikee_venue_photos_v8');
      if (cached) {
    ...
    fetch('/api/photos')
      ...
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          setVenuePhotos(data);
          try {
            localStorage.setItem('chikee_venue_photos_v8', JSON.stringify(data));
          } catch {}
        }
      })
      .catch(() => {
  ...
  const handleUpdateVenuePhotos = (newPhotos: VenuePhoto[]) => {
    ...
    try {
      localStorage.setItem('chikee_venue_photos_v8', JSON.stringify(newPhotos));
    } catch (e) {
